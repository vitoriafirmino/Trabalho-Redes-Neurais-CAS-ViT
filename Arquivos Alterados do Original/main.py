...
def main(args):
    ...
    if args.eval:
        ...
        test_stats, y_true, y_pred = evaluate(data_loader_val, model, device, use_amp=args.use_amp)
        
        #from sklearn.metrics import classification_report
        class_names = dataset_val.classes if hasattr(dataset_val, 'classes') else [str(i) for i in range(args.nb_classes)]
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            ...
            if args.model_ema and args.model_ema_eval:
                test_stats_ema, _, _ = evaluate(data_loader_val, model_ema.ema, device, use_amp=args.use_amp)
    ...
    if data_loader_val is not None:
        ...
        print("\nClassification Report Final (após todas as épocas):")
